...
def load_config_dict_from_yaml(config_fn):
    config_dict = yaml.safe_load(Path(config_fn).read_text())
    return config_dict
...
def check_config_against_directory(config_fn):
    ...
    if len(errors) == 0:
        print("--------------")
        print("No errors found - config matches directory")
        print("--------------")
        return True
    else:
        print("")
...
def check_property_against_assets(property_name, property_distribution_dict, property_folder):
    ...
    config_prop_names = list(property_distribution_dict['values'].keys())
